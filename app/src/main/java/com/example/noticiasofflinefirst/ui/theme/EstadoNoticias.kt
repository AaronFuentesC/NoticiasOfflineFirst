...
_estado.value = if (noticias.isEmpty()) {
EstadoNoticias.Error("No hay noticias disponibles")
} else {
EstadoNoticias.Exito(noticias)
}
} catch (e: Exception) {
_estado.value = EstadoNoticias.Error("Error al cargar noticias: ${e.message}")
}
}
}
}